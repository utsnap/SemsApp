package com.example.SemsApp.preference;

/**
 * Created by Administrator on 14. 3. 22.
 * 프리퍼런스에서 사용되는 키들을 모두 상수화 시켜 정의한다.
 * 추후에 변경이나 추가가 있을 수 있다.
 */
public class PreferenceKeys {
	public static final String MACHINE_SETTING_PASSWORD = "machine_setting_password";
	public static final String FIRST_PHONE_NUMBER = "first_phone_number";
	public static final String SECOND_PHONE_NUMBER = "second_phone_number";
	public static final String THIRD_PHONE_NUMBER = "third_phone_number";
	public static final String OLD_SEMS_USED = "old_sems_used";
	public static final String OLD_SEMS_PHONE_NUMBER = "old_sems_phone_number";
	public static final String NEW_SEMS_USED = "new_sems_used";
	public static final String NEW_SEMS_PHONE_NUMBER = "new_sems_phone_number";
	public static final String LED_DIMMER_USED = "led_dimmer_used";
	public static final String LED_DIMMER_PHONE_NUMBER = "led_dimmer_phone_number";
	public static final String CARBON_HEATER_USED = "carbon_machine_used";
	public static final String CARBON_HEATER_PHONE_NUMBER = "carbon_machine_phone_number";

	public static final String OLD_SEMS_EMERGENCY_CONTACT1 = "old_sems_emergency_contact1";
	public static final String OLD_SEMS_EMERGENCY_CONTACT2 = "old_sems_emergency_contact2";
	public static final String OLD_SEMS_EMERGENCY_CONTACT3 = "old_sems_emergency_contact3";
	public static final String OLD_SEMS_EMERGENCY_CONTACT4 = "old_sems_emergency_contact4";
	public static final String OLD_SEMS_EMERGENCY_CONTACT5 = "old_sems_emergency_contact5";

	public static final String FIRST_OLD_SEMS_DATA = "first_old_sems_data";
	public static final String SECOND_OLD_SEMS_DATA = "second_old_sems_data";
	public static final String THIRD_OLD_SEMS_DATA = "third_old_sems_data";
	public static final String FORTH_OLD_SEMS_DATA = "forth_old_sems_data";

	public static final String FIRST_NEW_SEMS_DATA = "first_new_sems_data";
	public static final String SECOND_NEW_SEMS_DATA = "second_new_sems_data";
	public static final String THIRD_NEW_SEMS_DATA = "third_new_sems_data";
	public static final String FORTH_NEW_SEMS_DATA = "forth_new_sems_data";

	public static final String FIRST_LED_DIMMER_DATA = "first_led_dimmer_data";
	public static final String SECOND_LED_DIMMER_DATA = "second_led_dimmer_data";
	public static final String THIRD_LED_DIMMER_DATA = "third_led_dimmer_data";
	public static final String FORTH_LED_DIMMER_DATA = "forth_led_dimmer_data";

	public static final String FIRST_CARBON_HEATER_DATA = "first_carbon_heater_data";
	public static final String SECOND_CARBON_HEATER_DATA = "second_carbon_heater_data";
	public static final String THIRD_CARBON_HEATER_DATA = "third_carbon_heater_data";
	public static final String FORTH_CARBON_HEATER_DATA = "forth_carbon_heater_data";

}
